...
abstract public class ToStream extends SerializerBase {
    ...
    /**
     * Stack to keep track of whether or not we need to preserve whitespace. Used to push/pop values
     * used for the field m_ispreserve, but m_ispreserve is only relevant if m_doIndent is true. If
     * m_doIndent is false this field has no impact.
     */
    protected BoolStack m_preserves = new BoolStack();

    /**
     * State flag to tell if preservation of whitespace is important. Used only in shouldIndent()
     * but only if m_doIndent is true. If m_doIndent is false this flag has no impact.
     */
    protected boolean m_ispreserve = false;

    /**
     * State flag that tells if the previous node processed was text, so we can tell if we should
     * preserve whitespace. Used in endDocument() and shouldIndent() but only if m_doIndent is true.
    ...
    /**
     * Flag to signal that a newline should be added. Used only in indent() which is called only if
     * m_doIndent is true. If m_doIndent is false this flag has no impact.
     */
     ...
     * <p>
     * The content model will consist of the string "EMPTY", the string "ANY", or a parenthesised
    ...
    @Override
    void setProp(final String name, String val, final boolean defaultVal) {
        if (val != null) {

            final char first = getFirstCharLocName(name);
            switch (first) {
                case 'c':
                    if (OutputKeys.CDATA_SECTION_ELEMENTS.equals(name)) {
                        final String cdataSectionNames = val;
                ...
                case 'e':
                    ...
                    if (OutputKeys.ENCODING.equals(name)) {
                        final String possible_encoding = Encodings.getMimeEncoding(val);
                        ...
                        if ((defaultVal
                                && (oldDefaultEncoding == null || !oldDefaultEncoding.equalsIgnoreCase(newEncoding)))
                                || (!defaultVal && (oldExplicitEncoding == null
                                        || !oldExplicitEncoding.equalsIgnoreCase(newEncoding)))) {
                            ...
                            if (newEncoding != null && encodingInfo.name == null) {
                                ...
                                final String msg = Utils.messages
                                        .createMessage(MsgKey.ER_ENCODING_NOT_SUPPORTED, new Object[] { newEncoding });
                                ...
                                val = Encodings.DEFAULT_MIME_ENCODING; // to store the modified
                            ...
                            if (defaultVal == false || oldExplicitEncoding == null) {
                                ...
                                if (os != null) {
                                    ...
                                    final String oldEncoding = getOutputProperty(OutputKeys.ENCODING);
                ...
                case 'i':
                    if (OutputPropertiesFactory.S_KEY_INDENT_AMOUNT.equals(name)) {
                        ...
                        final boolean b = "yes".equals(val) ? true : false;
                        m_doIndent = b;
                    }
    ...
    public void setOutputFormat(final Properties format) {
        ...
        final boolean shouldFlush = m_shouldFlush;
        ...
        if (format != null) {
            ...
            while (propNames.hasMoreElements()) {
                ...
                final String explicitValue = (String) format.get(key);
        ...
        final String entitiesFileName = (String) format.get(OutputPropertiesFactory.S_KEY_ENTITIES);

        if (null != entitiesFileName) {

            final String method = (String) format.get(OutputKeys.METHOD);
    ...
    private void setWriterInternal(final Writer writer, final boolean setByUser) {
     ...
     *            True if an input NL is replaced with the operating systems end-of-line separator.
     * @return The previously set value of the serializer.
     */
    public boolean setLineSepUse(final boolean use_sytem_line_break) {
        final boolean oldValue = m_lineSepUse;
     ...
     * <p>
     * The encoding specified in the output properties is used, or if no encoding was specified, the
     * default for the selected output method.
     *
     * @param output
     *            The output stream
     */
    ...
    private void setOutputStreamInternal(final OutputStream output, final boolean setByUser) {
        ...
        if (Encodings.DEFAULT_MIME_ENCODING.equalsIgnoreCase(encoding)) {
            ...
            final Writer osw = new OutputStreamWriter(output);
            setWriterInternal(osw, false);
        }
    }
    ...
    protected void indent(final int depth) throws IOException {
        ...
        /*
         * For m_indentAmount > 0 this extra test might be slower but Xalan's default value is 0, so
         * this extra test will run faster in that situation.
         */
     ...
     * <p>
     * Only the effective (first) declaration for an attribute will be reported. The type will be
     ...
     * @param valueDefault
     *            A string representing the attribute default ("#IMPLIED", "#REQUIRED", or "#FIXED")
     *            or null if none of these applies.
     * @param value
    ...
    protected boolean escapingNotNeeded(final char ch) {
        ...
        if (ch < 127) {
            ...
            if (ch >= CharInfo.S_SPACE || (CharInfo.S_LINEFEED == ch || CharInfo.S_CARRIAGERETURN == ch
                    || CharInfo.S_HORIZONAL_TAB == ch)) ret = true;
            else ret = false;
        } else {
     ...
     * <p>
     * An exception is thrown if there is no low surrogate in the pair, because the array ends
     * unexpectely, or if the low char is there but its value is such that it is not a low
     * surrogate.
     *
     * @param c
     *            the first (high) part of the surrogate, which must be confirmed before calling
     *            this method.
     * @param ch
     *            Character array.
     * @param i
     ...
     *         character represented by the surrogate pair if an entity reference with that value
     *         was written out.
     * @throws IOException
     * @throws org.xml.sax.SAXException
    ...
    protected int writeUTF16Surrogate(final char c, final char ch[], final int i, final int end) throws IOException {
        ...
        if (i + 1 >= end) {
            throw new IOException(
                    Utils.messages
                            .createMessage(MsgKey.ER_INVALID_UTF16_SURROGATE, new Object[] { Integer.toHexString(c) }));
        }
     ...
     *            length of chars.
     * @param fromTextNode
     *            true if the characters being processed are from a text node, false if they are
    ...
    int accumDefaultEntity(
            ...
            final char[] chars,
            final int len,
            final boolean fromTextNode,
            final boolean escLF) throws IOException {
        ...
        if (!escLF && CharInfo.S_LINEFEED == ch) {
            ...
            if ((fromTextNode && m_charInfo.shouldMapTextChar(ch))
                    || (!fromTextNode && m_charInfo.shouldMapAttrChar(ch))) {
                final String outputStringForChar = m_charInfo.getOutputStringForChar(ch);
     ...
     *            true if the operating systems end-of-line separator should be output rather than a
     *            new-line character.
     * @throws IOException
     * @throws org.xml.sax.SAXException
     */
    void writeNormalizedChars(
            ...
            final boolean useSystemLineSeparator) throws IOException, org.xml.sax.SAXException {
        final java.io.Writer writer = m_writer;
     ...
     * <p>
     * The contents of the un-escaping section will be delivered through the regular
     ...
     * </p>
     * <p>
     * Note that some parsers will report whitespace using the ignorableWhitespace() method rather
     ...
     * @throws org.xml.sax.SAXException
     */
    protected void cdata(final char ch[], final int start, final int length) throws org.xml.sax.SAXException {
        ...
        try {
            ...
            /*
             * Write out the CDATA opening delimiter only if we are supposed to, and if we are not
             * already in the middle of a CDATA section
             */
            ...
            /*
             * used to always write out CDATA closing delimiter here, but now we delay, so that we
             * can merge CDATA sections on output. need to write closing delimiter later
             */
            if (writeCDataBrackets) {
                /*
                 * if the CDATA section ends with ] don't leave it open as there is a chance that an
                 * adjacent CDATA sections starts with ]>. We don't want to merge ]] with > , or ]
                 * with ]>
                 */
     ...
     * @throws org.xml.sax.SAXException
     */
    protected void charactersRaw(final char ch[], final int start, final int length) throws org.xml.sax.SAXException {
    ...
    public void characters(final char chars[], final int start, final int length) throws org.xml.sax.SAXException {
        ...
        if (m_cdataStartCalled || m_elemContext.m_isCdataSection) {
            /*
             * either due to startCDATA() being called or due to cdata-section-elements atribute, we
             * need this as cdata
             */
        ...
        try {
            ...
            while (i < end && isAllWhitespace) {
                ...
                if (m_charInfo.shouldMapTextChar(ch1)) {
                    ...
                    final String outputStringForChar = m_charInfo.getOutputStringForChar(ch1);
                    writer.write(outputStringForChar);
            ...
            /*
             * If there is some non-whitespace, mark that we may need to preserve this. This is only
             * important if we have indentation on.
             */
    ...
    private void writeOutCleanChars(final char[] chars, final int i, final int lastProcessed) throws IOException {
        int startClean;
    ...
    private int processDirty(
            ...
            final char ch,
            final int lastDirty,
            final boolean fromTextNode) throws IOException {
     ...
     */
    @Override
    public void characters(final String s) throws org.xml.sax.SAXException {
     ...
     *            length of chars.
     * @param fromTextNode
     *            true if the characters being processed are from a text node, false if the
        ...
        if (i == pos) {
            ...
            if (fromTextNode && Encodings.isHighUTF16Surrogate(ch)) {
                ...
                if (i + 1 >= len) {
                    ...
                    if (!(Encodings.isLowUTF16Surrogate(next))) throw new IOException(
                            Utils.messages
                                    .createMessage(
                                            MsgKey.ER_INVALID_UTF16_SURROGATE,
                                            new Object[] {
            ...
            if (!wasProcessedAsSurrogates) {
                 ...
                 * Numeric Character Reference(NCR) regardless of XML Version being used for output
                 * document.
                 */
    ...
    public void startElement(
        ...
        try {
            ...
            /*
             * before we over-write the current elementLocalName etc. lets close out the old one (if
             * we still need to)
             */
            if (m_elemContext.m_startTagOpen) {
                closeStartTag();
     ...
     *
     * @param name
     *            non-null reference to document type name. NEEDSDOC @param closeDecl
     * @throws java.io.IOException
     */
    void outputDocTypeDecl(final String name, boolean closeDecl) throws SAXException {
        ...
        try {
            ...
            final String doctypePublic = getDoctypePublic();
            ...
            final String doctypeSystem = getDoctypeSystem();
     ...
     * @throws org.xml.sax.SAXException
     */
    public void processAttributes(final java.io.Writer writer, final int nAttrs) throws IOException, SAXException {
        /*
         * real SAX attributes are not passed in, so process the attributes that were collected
         * after the startElement call. _attribVector is a "cheap" list for Stream serializer output
    ...
    public void endElement(
        ...
        try {
            ...
            if (m_elemContext.m_startTagOpen) {
                ...
                /*
                 * don't need to pop cdataSectionState because this element ended so quickly that we
                 * didn't get to push the state.
                 */
     ...
     *            The Namespace URI the prefix is mapped to.
     * @param shouldFlush
     *            true if any open tags need to be closed first, this will impact which element the
     *            mapping applies to (open parent, or its up comming child)
    ...
    public boolean startPrefixMapping(
            final String prefix,
            final String uri,
            final boolean shouldFlush) throws org.xml.sax.SAXException {

        /*
         * Remember the mapping, and at what depth it was declared This is one greater than the
         * current depth because these mappings will apply to the next depth. This is in
        ...
        if (pushed) {
            /*
             * Brian M.: don't know if we really needto do this. The callers of this object should
             * have injected both startPrefixMapping and the attributes. We are just covering our
             * butt here.
             */
            String name;
            if (EMPTYSTRING.equals(prefix)) {
                ...
                { // that maps ns1 prefix to "" URI
                    ...
                    /*
                     * for something like xmlns:abc="w3.pretend.org" the uri is the value, that is
                     * why we pass it in the value, or 5th slot of addAttributeAlways()
                     */
     ...
     */
    @Override
    public void comment(final char ch[], int start, final int length) throws org.xml.sax.SAXException {
        ...
        /*
         * Don't write out any indentation whitespace now, because there may be non-whitespace text
         * after this. Simply mark that at this point if we do decide to indent that we should add a
         * newline on the end of the current line before the indentation at the start of the next
         * line.
         */
        m_startNewLine = true;
     ...
     */
    @Override
    public void endPrefixMapping(final String prefix) throws org.xml.sax.SAXException { // do
                                                                                        // nothing
    }

    /**
     * Receive notification of ignorable whitespace in element content. Not sure how to get this
     * invoked quite yet.
     *
     * @param ch
     *            The characters from the XML document.
     ...
     * @param name
     *            The name of the skipped entity. If it is a parameter entity, the name will begin
     *            with '%', and if it is the external DTD subset, it will be the string "[dtd]".
     * @throws org.xml.sax.SAXException
     *             Any SAX exception, possibly wrapping another exception.
     */
    @Override
    public void skippedEntity(final String name) throws org.xml.sax.SAXException { // TODO: Should
                                                                                   // handle
    }
    ...
    /**
     * Report the beginning of an entity. The start and end of the document entity are not reported.
     * The start and end of the external DTD subset are reported using the pseudo-name "[dtd]". All
     * other events must be properly nested within start/end entity events.
     *
     * @param name
     *            The name of the entity. If it is a parameter entity, the name will begin with '%'.
     * @throws org.xml.sax.SAXException
    ...
    public void startEntity(final String name) throws org.xml.sax.SAXException {
        ...
        if (!m_expandDTDEntities && !m_inExternalDTD) {
            /*
             * Only leave the entity as-is if we've been told not to expand them and this is not the
             * magic [dtd] name.
             */
            startNonEscaping();
            characters("&" + name + ';');
    ...
    protected void closeStartTag() throws SAXException {
        ...
        if (m_elemContext.m_startTagOpen) {
            ...
            /*
             * whether Xalan or XSLTC, we have the prefix mappings now, so lets determine if the
             * current element is specified in the cdata- section-elements list.
             */
    ...
    /**
     * Report the start of DTD declarations, if any. Any declarations are assumed to be in the
     * internal subset unless otherwise indicated.
     *
     * @param name
     *            The document type name.
     * @param publicId
     *            The declared public identifier for the external DTD subset, or null if none was
     *            declared.
     * @param systemId
     *            The declared system identifier for the external DTD subset, or null if none was
     *            declared.
     * @throws org.xml.sax.SAXException
     *             The application may raise an exception.
     * @see #endDTD
    ...
    public void startDTD(
            final String name,
            final String publicId,
            final String systemId) throws org.xml.sax.SAXException {
     ...
     *            the property key.
     * @param props
     *            the list of properties to search in. Sets the vector of local-name/URI pairs of
     *            the cdata section elements specified in the cdata-section-elements property. This
     *            method is essentially a copy of getQNameProperties() from OutputProperties.
     *            Eventually this method should go away and a call to setCdataSectionElements(Vector
    ...
    protected String ensureAttributesNamespaceIsDeclared(
            final String ns,
            final String localName,
            final String rawName) throws org.xml.sax.SAXException {
        ...
        if (ns != null && ns.length() > 0) {
            ...
            if (index > 0) {
                ...
                final String uri = m_prefixMap.lookupNamespace(prefixFromRawName);
    ...
    void ensurePrefixIsDeclared(final String ns, final String rawName) throws org.xml.sax.SAXException {
        ...
        if (ns != null && ns.length() > 0) {
            ...
            if (null != prefix) {
                final String foundURI = m_prefixMap.lookupNamespace(prefix);
     ...
     * the element name is seen. This method is a copy of its super classes method, except that some
     * tracing of events is done. This is so the tracing is only done for stream serializers, not
     * for SAX ones.
     *
     * @param uri
    ...
    public boolean addAttributeAlways(
        ...
        if (index >= 0) {
            ...
            /*
             * We've seen the attribute before. We may have a null uri or localName, but all we
             * really want to re-set is the value anyway.
             */
            ...
            if (xslAttribute) {
                /*
                 * This attribute is from an xsl:attribute element so we take some care in adding
                 * it, e.g. <elem1 foo:attr1="1" xmlns:foo="uri1"> <xsl:attribute
                 * name="foo:attr2">2</xsl:attribute> </elem1> We are adding attr1 and attr2 both as
                 * attributes of elem1, and this code is adding attr2 (the xsl:attribute ). We could
                ...
                if (colonIndex > 0) {
                    ...
                    /*
                     * Before adding this attribute (foo:attr2), is the prefix for it (foo) already
                     * mapped at the current depth?
                     */
                    if (existing_mapping != null
                            && existing_mapping.m_declarationDepth == m_elemContext.m_currentElemDepth
                            && !existing_mapping.m_uri.equals(uri)) {
                        /*
                         * There is an existing mapping of this prefix, it differs from the one we
                         * need, and unfortunately it is at the current depth so we can not
                         * over-ride it.
                         */
                        ...
                        if (prefix == null) {
                            /*
                             * Unfortunately there is no existing prefix that happens to map to
                             * ours, so to avoid a prefix collision we must generated a new prefix
                             * to use. This is OK because the prefix URI mapping defined in the
                             * xsl:attribute is short in scope, just the xsl:attribute element
                             * itself, and at this point in serialization the body of the
                             * xsl:attribute, if any, is just a String. Right? . . . I sure hope so
                             * - Brian M.
                             */
                            prefix = m_prefixMap.generateNextPrefix();
                        }
                ...
                try {
                    /*
                     * This is our last chance to make sure the namespace for this attribute is
                     * declared, especially if we just generated an alternate prefix to avoid a
                     * collision (the new prefix/rawName will go out of scope soon and be lost ...
                     * last chance here.
                     */
    ...
    public void setTransformer(final Transformer transformer) {
        ...
        if (m_tracer != null && !(m_writer instanceof SerializerTraceWriter)) setWriterInternal(
                new SerializerTraceWriter(m_writer, m_tracer),
                false);
    ...
    private void resetToStream() {
        ...
        /*
         * The stream is being reset. It is one of ToXMLStream, ToHTMLStream ... and this type can't
         * be changed so neither should m_charInfo which is associated with the type of Stream. Just
         * leave m_charInfo as-is for the next re-use.
         */
    ...
    /**
     * Simple stack for boolean values. This class is a copy of the one in org.apache.xml.utils. It
     * exists to cut the serializers dependancy on that package. A minor changes from that package
    ...
    static final class BoolStack {
        ...
        private void grow() {
            ...
            final boolean newVector[] = new boolean[m_allocatedSize];
     ...
     *
     * @param URI_and_localNames
     *            a whitespace separated list of element names, each element is a URI in curly
     *            braces (optional) and a local name. An example of such a parameter is:
